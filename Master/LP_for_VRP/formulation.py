...
# 双対問題を解く関数の定義
def solve_dual(As, Ac_eq, Ac_leq, Ar_eq, Ar_leq, Ap, Aq, b_eq, b_leq, op, oq, instance_name, num_vars, VBases, CBases):
    import gurobipy as gp
    from gurobipy import GRB
    import numpy as np
...
def solve_dual_2(As, Ac_eq, Ac_leq, Ar_eq, Ar_leq, Ap, Aq, b_eq, b_leq, op, oq, instance_name, num_vars_1, num_vars_2, PStarts, DStarts):
#def solve_dual_2(As, Ac_eq, Ac_leq, Ar_eq, Ar_leq, Ap, Aq, b_eq, b_leq, op, oq, instance_name, num_vars_1, num_vars_2, VBases, CBases):
    import gurobipy as gp
    from gurobipy import GRB
    import numpy as np
    ...
    #assert(len(m.getVars()) == len(VBases))
    #assert(len(m.getConstrs()) == len(CBases))
    # ホットスタートの使用
    for i, var in enumerate(m.getVars()):
        var.PStart = PStarts[i]
        #var.VBasis = -1 #int(VBases[i])
    for i, constr in enumerate(m.getConstrs()):
        constr.DStart = DStarts[i]
        #constr.CBasis = -1 #int(CBases[i])
